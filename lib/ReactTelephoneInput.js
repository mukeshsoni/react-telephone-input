...
var _extends = Object.assign || function (target) { for (var i = 1; i < arguments.length; i++) { var source = arguments[i]; for (var key in source) { if (Object.prototype.hasOwnProperty.call(source, key)) { target[key] = source[key]; } } } return target; }; // TODO - fix the onlyContries props. Currently expects that as an array of country object, but users should be able to send in array of country isos

exports.formatNumber = formatNumber;
exports.isNumberValid = isNumberValid;
exports.replaceCountryCode = replaceCountryCode;
...
function formatNumber(text, pattern, autoFormat) {
  ...
  var formattedObject = pattern.split("").reduce(function (acc, character) {
    ...
    return {
      formattedText: acc.formattedText + (0, _cramda.first)(acc.remainingText),
      remainingText: (0, _cramda.tail)(acc.remainingText)
    };
...
function isNumberValid(inputNumber) {
  ...
  return (0, _cramda.any)(function (country) {
    return (0, _cramda.startsWith)(country.dialCode, inputNumber) || (0, _cramda.startsWith)(inputNumber, country.dialCode);
  }, countries);
}
...
var ReactTelephoneInput = exports.ReactTelephoneInput = createReactClass({
  ...
  getInitialState: function getInitialState() {
    ...
    return Object.assign({}, {
      ...
      debouncedQueryStingSearcher: (0, _debounce2.default)(this.searchCountry, 300)
  ...
  shouldComponentUpdate: function shouldComponentUpdate(nextProps, nextState) {
    return !(0, _cramda.equals)(nextProps, this.props) || !(0, _cramda.equals)(nextState, this.state);
  },
  ...
  guessSelectedCountry: function guessSelectedCountry(inputNumber) {
    ...
    var secondBestGuess = (0, _cramda.find)((0, _cramda.propEq)("iso2", defaultCountry), allCountries) || this.props.onlyCountries[0];
    ...
    if (inputNumber.trim() !== "") {
      var bestGuess = onlyCountries.reduce(function (selectedCountry, country) {
        ...
        if (allCountryCodes[inputNumberForCountries] && allCountryCodes[inputNumberForCountries][0] === country.iso2) {
          ...
          if ((0, _cramda.startsWith)(country.dialCode, inputNumber)) {
  ...
  handleFlagDropdownClick: function handleFlagDropdownClick(e) {
    ...
    var onlyCountries = this.props.onlyCountries;

    // need to put the highlight on the current selected country if the dropdown is going to open up

    this.setState({
      showDropDown: !this.state.showDropDown,
      highlightCountry: (0, _cramda.find)((0, _cramda.equals)(this.state.selectedCountry), this.props.onlyCountries),
      highlightCountryIndex: (0, _cramda.findIndex)((0, _cramda.propEq)("iso2", selectedCountry.iso2), preferredCountries.concat(onlyCountries))
    }, function () {
  ...
  handleFlagItemClick: function handleFlagItemClick(country) {
    ...
    var nextSelectedCountry = (0, _cramda.find)((0, _cramda.equals)(country), this.props.onlyCountries);
  ...
  _mapPropsToState: function _mapPropsToState(props) {
    ...
    var selectedCountryGuessIndex = (0, _cramda.findIndex)((0, _cramda.propEq)("iso2", selectedCountryGuess.iso2), allCountries);
  ...
  _searchCountry: (0, _lodash2.default)(function (queryString) {
    if (!queryString || queryString.length === 0) {
      return null;
    }
    // don't include the preferred countries in search
    var probableCountries = this.props.onlyCountries.filter(function (country) {
      return (0, _cramda.startsWith)(queryString.toLowerCase(), country.name.toLowerCase());
    }, this);
  ...
  searchCountry: function searchCountry() {
    ...
    var probableCandidateIndex = (0, _cramda.findIndex)((0, _cramda.propEq)("iso2", probableCandidate.iso2), this.props.onlyCountries) + this.state.preferredCountries.length;
  ...
  getCountryDropDownList: function getCountryDropDownList() {
    ...
    var countryDropDownList = this.state.preferredCountries.concat(this.props.onlyCountries).map(function (country, index) {
      var itemClasses = classNames({
        country: true,
        preferred: (0, _cramda.findIndex)((0, _cramda.propEq)("iso2", country.iso2), self.state.preferredCountries) >= 0,
