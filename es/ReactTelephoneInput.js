...
import { any, find, propEq, equals, findIndex, first, tail, startsWith } from "./cramda";
...
export function isNumberValid(inputNumber) {
  ...
  return any(function (country) {
    return startsWith(country.dialCode, inputNumber) || startsWith(inputNumber, country.dialCode);
  }, countries);
}
...
export var ReactTelephoneInput = createReactClass({
  ...
  guessSelectedCountry: function guessSelectedCountry(inputNumber) {
    ...
    var secondBestGuess = find(propEq("iso2", defaultCountry), allCountries) || this.props.onlyCountries[0];
  ...
  searchCountry: function searchCountry() {
    ...
    var probableCandidateIndex = findIndex(propEq("iso2", probableCandidate.iso2), this.props.onlyCountries) + this.state.preferredCountries.length;
